...
Create and review breast-region and nipple masks with an AI agent and 3D Slicer.

[All skills](../../README.md) Â· [Agent instructions](SKILL.md)
...
```text
$skill-installer install the breast-mri-annotation skill from https://github.com/maystroh/agents_skills/tree/main/breast-mri-annotation/skill
```
...
Clone this repository, then copy the **whole** `breast-mri-annotation/skill` folder, naming the installed destination `breast-mri-annotation` into your personal skills directory. The following commands use the user-level `.agents/skills` location documented for Codex. Use one installation method to avoid duplicate copies of the same skill.
...
Copy-Item -LiteralPath '.\agents_skills\breast-mri-annotation\skill' -Destination $skillDest -Recurse
```
...
if [ -e "$HOME/.agents/skills/breast-mri-annotation" ]; then
  ...
  cp -R agents_skills/breast-mri-annotation/skill "$HOME/.agents/skills/breast-mri-annotation"
fi
```
...
```bash
python breast-mri-annotation/skill/scripts/batch_io.py inventory /path/to/mri-folder
```
...
â”œâ”€â”€ SKILL.md
â”œâ”€â”€ references/
â”‚   â”œâ”€â”€ annotation-rules.md
â”‚   â”œâ”€â”€ review1-learned-preferences.md
â”‚   â””â”€â”€ slicer-workflow.md
â””â”€â”€ scripts/
    â””â”€â”€ batch_io.py
```
