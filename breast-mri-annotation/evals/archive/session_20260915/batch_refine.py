...
for record in json.loads((batch/'batch.json').read_text())['cases']:
    ...
    for k in range(a.shape[2]):
        ...
        for j in range(low,high+1):
            ...
            x=int(xs[0]);left[j]=x;right[j]=int(xs[-1])
            ...
            if strength.max()>0:
                ...
                shifts[j]=edge-1-x
        shifts=gaussian_filter1d(shifts,1.3)
        for j in range(low,high+1):
    ...
    m['correction']={'row_span':spans[m['case'][-4:]],'breast_changed_voxels':int(np.count_nonzero(breast!=old['breast'])),'nipple_centers_ijk':centers}
    (run/'manifest.json').write_text(json.dumps(m,indent=2));print(m['case'],m['correction']['breast_changed_voxels'])
shutil.copy2(root/'batch_refine.py',batch/'correction_script.py')
finished=batch/'slicer_batch_finished.txt'
if finished.exists():finished.rename(batch/'slicer_batch_first_pass_finished.txt')
